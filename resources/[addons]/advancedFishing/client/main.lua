            ...
            if #(v.pedCoord - pos) < 3 then 
                ...
                if #(v.pedCoord - pos) < 2 then 
                    if IsControlJustReleased(0, 38) then 
                        SetNuiFocus(true,true)
                        if first then 
                            ESX.TriggerServerCallback("ak4y-advancedFishing:getPlayerDetails", function(result)
                                first = false
                                fullName = result.charInfo or ""
                                SendNUIMessage({
...
function openMenu()
    if menuOpenSpamProtection < GetGameTimer() then 
		...
		ESX.TriggerServerCallback("ak4y-advancedFishing:getPlayerDetails", function(result)
			...
			if first then 
				first = false
                local firstName = result.charInfo or ""
                fullName = firstName
				SendNUIMessage({
					type = 'openUiFirst',
					userDetails = result.resultData,
...
RegisterNUICallback('takeRewards', function(data, cb)
	if takeRewardsSpamProtect < GetGameTimer() then 
		...
		ESX.TriggerServerCallback("ak4y-advancedFishing:rewards", function(result)
			cb(result)
            if result then 
...
RegisterNUICallback('buyItem', function(data, cb)
    if buyItemSpamProtectx < GetGameTimer() then 
        ...
        ESX.TriggerServerCallback("ak4y-advancedFishing:buyItem", function(result)
            cb(result)
        end, data)
    end
end)
...
RegisterNUICallback('sellItem', function(data, cb)
    if sellItemSpamProtectx < GetGameTimer() then 
        ...
        ESX.TriggerServerCallback("ak4y-advancedFishing:sellItem", function(result)
            cb(result)
        end, data)
...
RegisterNUICallback('upgradeRod', function(data, cb)
	if upgradeSpamProtect < GetGameTimer() then 
		...
		ESX.TriggerServerCallback("ak4y-advancedFishing:upgradeRod", function(result)
			cb(result)
		end, data)
	end
end)
...
addXP = function(xp)
	ESX.TriggerServerCallback("ak4y-advancedFishing:addXP", function(result)
		if result then 
    ...
    if not fishing then
        ...
        if not IsPedInAnyVehicle(playerPed) and not IsPedSwimming(playerPed) then
            ...
            if zoneData then
                ...
                ESX.ShowNotification(Config.Language.youCantFishHere)
            end
        else
            ESX.ShowNotification(Config.Language.somethingStoppingFish)
        end
    end
end)
    ...
    while true do
        ...
        if fishing then
            if not pressKey then
                ...
                catching = false
                if fishing then
                    ESX.ShowNotification(Config.Language.movementOnRod)
                    Citizen.Wait(1000)
...
function fishingStop(bool)
    if fishing then
        ...
        pressKeyNow = false
        if bool then
            ESX.ShowNotification(Config.Language.missedFish, "error")
            if math.random(1,2) == 1 and bait ~= "none" then
                bait = "none"
                SendNUIMessage({type = "setBait", bait = "none"})
                ESX.ShowNotification(Config.Language.noLeftBait, "error")
            end
        else
            ...
            local playerPed = PlayerPedId()
            fishing = false
            ESX.ShowNotification(Config.Language.fishingHBStopped, "error")
...
RegisterNUICallback('successGame', function(data, cb)
    if successSpam < GetGameTimer() then 
        ...
        if tutulanBalik >= Config.Settings.addxpafterXcatch then
            ...
            ESX.ShowNotification("After "..tutulanBalik.." successful catching you get "..kazanilanXP.."XP!")
            addXP(kazanilanXP)
            tutulanBalik = 0
        end
        getLevel()
        ...
        if bait ~= "none" and myChance < Config.Settings.eatBaitChance then 
            bait = "none"
            ESX.ShowNotification(Config.Language.eatBaitText)
            SendNUIMessage({type = "setBait", bait = "none"})
        end

        if randomItem ~= nil then
            ESX.ShowNotification(randomItem .. " " .. Config.Language.catchFish)
    ...
    if fishing then 
        ...
        elseif baitType == "illegal" then
            ...
            if illegal then 
                ...
                ESX.ShowNotification(Config.Language.illegalCant)
            end
        end
    else
        ESX.ShowNotification(Config.Language.useBaitNotFishing)
    end
end)
