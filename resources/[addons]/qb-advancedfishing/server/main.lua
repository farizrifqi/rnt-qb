...
QBCore.Functions.CreateCallback('qb-advancedFishing:getPlayerDetails', function(source, cb)
    ...
    elseif Config.sql == "ghmattimysql" then
        exports.ghmattimysql:execute("SELECT * FROM ak4y_fishing WHERE citizenid = @citizenid", {
            ...
            if result[1] == nil then
                ...
                "INSERT INTO ak4y_fishing SET citizenid = @citizenid, currentXP = @currentXP, tasks = @tasks", {
                    ["@citizenid"] = citizenId,
                    ["@currentXP"] = 0,
                    ["@tasks"] = json.encode(tasks),
                })
    ...
    elseif Config.sql == "mysql-async" then
        MySQL.Async.fetchAll("SELECT * FROM ak4y_fishing WHERE citizenid = @citizenid", {
            ...
            if result[1] == nil then
                MySQL.Async.execute(
                'INSERT INTO ak4y_fishing (citizenid, currentXP, tasks) VALUES (@citizenid, @currentXP, @tasks)', {
                    ["@citizenid"] = citizenId,
                    ["@currentXP"] = 0,
                    ["@tasks"] = json.encode(tasks)
                })
        ...
        if result[1] then
            ...
            if not resultak4y[taskId].taken then
                if resultak4y[taskId].hasCount >= resultak4y[taskId].requiredCount then
                    ...
                    MySQL.update(
                    'UPDATE ak4y_fishing SET tasks = ?, time = (CURDATE() + INTERVAL ' ..
                    Config.Settings.tasksResetDay .. ' DAY) WHERE citizenid = ?', { json.encode(resultak4y), citizenId })
                    cb(true)
                else
                    cb(false)
                end
            ...
            if result[1] then
                ...
                if not resultak4y[taskId].taken then
                    if resultak4y[taskId].hasCount >= resultak4y[taskId].requiredCount then
                        ...
                        xPlayer.Functions.AddMoney('cash', money)
                        exports.ghmattimysql:execute(
                        "UPDATE ak4y_fishing SET tasks = @tasks, time = (CURDATE() + INTERVAL " ..
                        Config.Settings.tasksResetDay .. " DAY) WHERE citizenid = @citizenid", {
                            ['@citizenid'] = citizenId,
...
AddEventHandler('qb-advancedFishing:addTaskCount', function(itemName)
    ...
    if Config.sql == "oxmysql" then
        ...
        if result[1] then
            ...
            if kayit then
                MySQL.update('UPDATE ak4y_fishing SET tasks = ? WHERE citizenid = ?',
                    { json.encode(resultak4y), citizenId })
            end
        end
    elseif Config.sql == "ghmattimysql" then
        exports.ghmattimysql:execute("SELECT tasks FROM ak4y_fishing WHERE citizenid = @citizenid", {
...
QBCore.Functions.CreateCallback('qb-advancedFishing:addXP', function(source, cb, amount)
    ...
    elseif Config.sql == "ghmattimysql" then
        exports.ghmattimysql:execute(
        "UPDATE ak4y_fishing SET currentXP = currentXP + @currentXP WHERE citizenid = @citizenid", {
            ['@citizenid'] = citizenId,
            ['@currentXP'] = deger,
        })
    elseif Config.sql == "mysql-async" then
        ...
        if xPlayer.Functions.AddItem(currentItem.itemName, buyItemCount) then
            if xPlayer.Functions.RemoveMoney('cash', requiredCash) then
                TriggerClientEvent("QBCore:Notify", src,
                    Config.Language.succesBuy .. ": " .. buyItemCount .. 'x - ' .. currentItem.itemName .. '!', "success")
                cb(true)
            else
                TriggerClientEvent("QBCore:Notify", src, Config.Language.youDontHaveMoney, "error")
                cb(false)
...
QBCore.Functions.CreateCallback('qb-advancedFishing:sellItem', function(source, cb, data)
    ...
    if currentAmount ~= nil and currentAmount.amount >= sellItemCount then
        ...
        TriggerClientEvent("QBCore:Notify", src,
            Config.Language.succesSold .. ": " .. sellItemCount .. 'x, ' ..
            currentItem.itemName .. ' - $' .. rewardsCash .. '!', "success")
