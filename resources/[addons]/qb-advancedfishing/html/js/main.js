...
$(document).on("keydown", function () {
    switch (event.keyCode) {
        case 27: // ESC
            $.post("https://qb-advancedFishing/closeMenu", JSON.stringify());
            ...
            if (gameActive) {
                keyGameStop();
                $.post("https://qb-advancedFishing/failGame", JSON.stringify());
            }
            break;
    }
});
...
$(document).on("click", ".sellItemSellButton", function () {
    ...
    if (itemCount > 0) {
        $.post(
            "https://qb-advancedFishing/sellItem",
            JSON.stringify({
                itemId: itemId,
                itemCount: itemCount,
            }),
            ...
            if (!userDetailss[i].taken && userDetailss[i].hasCount < element.requiredCount) {
                ...
                if (!userDetailss[i].taken) {
                    recentEvent =
                        recentEvent +
                        `
                                <div class="rightItem takeRewards" id="takeRewards-${i}" data-taskId="${element.taskId}" data-moneyRewards="${element.moneyRewards
                                    ...
                                    <div class="rightItemLevelSection">
                                        <div class="rightItemLevelFill" style="width:${(userDetailss[i].hasCount / element.requiredCount) * 100
                        }%"></div>
                                    </div>
                                </div>
                                `;
...
$(document).on("click", ".rightItem.takeRewards", function () {
    ...
    let divId = this.id;
    $.post(
        "https://qb-advancedFishing/takeRewards",
        JSON.stringify({
            taskId: taskId,
            money: moneyRewards,
            xp: xpRewards,
...
$(document).on("click", ".marketBuyButton", function () {
    ...
    if (itemCount > 0) {
        $.post(
            "https://qb-advancedFishing/buyItem",
            JSON.stringify({
                itemId: itemId,
                itemCount: itemCount,
            }),
...
$(document).on("click", ".upgradeRodButton", function () {
    $.post(
        "https://qb-advancedFishing/upgradeRod",
        JSON.stringify({
            hedef: hedef,
            price: selectedPrice,
        }),
...
function addRandomKey() {
    const key = new Key(randomLetter());
    ...
    if (keys.some((keyObj) => keyObj.key === key)) {
        ...
        if (correctCount >= 5) {
            if (gameActive) {
                ...
                setTimeout(() => {
                    keyGameStop();
                    $.post("https://qb-advancedFishing/successGame", JSON.stringify());
                }, 150);
            }
        }
    } else {
